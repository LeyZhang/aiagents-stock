...
class StockAnalysisAgents:
    ...
    def __init__(self, model="deepseek-chat"):
        logger.debug(f"初始化 StockAnalysisAgents: model={model}")
        self.model = model
    ...
    def technical_analyst_agent(self, stock_info: Dict, stock_data: Any, indicators: Dict) -> Dict[str, Any]:
        """技术面分析智能体"""
        logger.debug(f"technical_analyst_agent() 调用: symbol={stock_info.get('symbol', 'N/A')}")
    ...
    def fundamental_analyst_agent(self, stock_info: Dict, financial_data: Dict = None, quarterly_data: Dict = None) -> Dict[str, Any]:
        """基本面分析智能体"""
        logger.debug(f"fundamental_analyst_agent() 调用: symbol={stock_info.get('symbol', 'N/A')}")
        ...
        if quarterly_data and quarterly_data.get('data_success'):
            ...
            logger.debug(f"已获取季报数据：利润表{income_count}期，资产负债表{balance_count}期，现金流量表{cash_flow_count}期")
        else:
            logger.warning("未获取到季报数据，将基于基本财务数据分析")
    ...
    def fund_flow_analyst_agent(self, stock_info: Dict, indicators: Dict, fund_flow_data: Dict = None) -> Dict[str, Any]:
        """资金面分析智能体"""
        logger.debug(f"fund_flow_analyst_agent() 调用: symbol={stock_info.get('symbol', 'N/A')}")
        logger.info("资金面分析师正在分析中...")

        # 如果有资金流向数据，显示数据来源
        if fund_flow_data and fund_flow_data.get('data_success'):
            logger.debug("已获取资金流向数据（akshare数据源）")
        else:
            logger.warning("未获取到资金流向数据，将基于技术指标分析")
    ...
    def risk_management_agent(self, stock_info: Dict, indicators: Dict, risk_data: Dict = None) -> Dict[str, Any]:
        ...
        if risk_data and risk_data.get('data_success'):
            logger.debug("已获取问财风险数据（限售解禁、大股东减持、重要事件）")
        else:
            logger.warning("未获取到风险数据，将基于基本信息分析")
...
"""
    ...
    def market_sentiment_agent(self, stock_info: Dict, sentiment_data: Dict = None) -> Dict[str, Any]:
        ...
        if sentiment_data and sentiment_data.get('data_success'):
            logger.debug("已获取市场情绪数据（ARBR、换手率、涨跌停等）")
        else:
            logger.warning("未获取到详细情绪数据，将基于基本信息分析")
...
"""
    ...
    def news_analyst_agent(self, stock_info: Dict, news_data: Dict = None) -> Dict[str, Any]:
        ...
        if news_data and news_data.get('data_success'):
            ...
            logger.debug(f"已从 {source} 获取 {news_count} 条新闻")
        else:
            logger.warning("未获取到新闻数据，将基于基本信息分析")
