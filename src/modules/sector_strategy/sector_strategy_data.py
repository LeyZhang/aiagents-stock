...
logger = get_logger('sector_strategy_data')
        ...
        try:
            ...
            if hasattr(self, '_tushare_api') and self._tushare_api:
                ...
                if df is not None and not df.empty:
                    ...
                    logger.debug("    [Tushare] ❌ 未获取到数据")
            else:
                logger.debug("    [Tushare] 不可用")
        except Exception as e:
            logger.debug(f"    [Tushare] 获取北向资金失败: {e}")
...
""")
    ...
    def get_cached_data_with_fallback(self):
        ...
        try:
            ...
            # 如果获取失败，回退到缓存数据
            logger.info("[智策] 获取最新数据失败，尝试加载缓存数据...")
