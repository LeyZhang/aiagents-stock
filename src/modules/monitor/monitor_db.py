...
class StockMonitorDatabase:
    ...
    def __init__(self, db_path: str = "data/db/stock_monitor.db"):
    ...
    def batch_add_or_update_monitors(self, monitors_data: List[Dict]) -> Dict[str, int]:
        ...
        for data in monitors_data:
            try:
                ...
                if not symbol or not all([entry_min, entry_max, take_profit, stop_loss]):
                    logger.warning(f"{symbol} 参数不完整，跳过")
                    failed += 1
                    continue
                ...
                if existing:
                    ...
                    added += 1
                    logger.info(f"添加监测: {symbol}")

            except Exception as e:
                symbol_str = data.get('code') or data.get('symbol', 'Unknown')
                logger.error(f"处理监测失败 ({symbol_str}): {str(e)}")
                failed += 1
        ...
        logger.info(f"批量同步完成: 新增{added}只, 更新{updated}只, 失败{failed}只")
        return result
