...
class DataSourceManager:
    ...
    def __init__(self):
        ...
        logger.debug(f"初始化数据源管理器: tushare_token={bool(self.tushare_token)}")

        # 初始化tushare
        if self.tushare_token:
            try:
                logger.debug(f"正在初始化Tushare，Token前缀: {self.tushare_token[:10]}...")
    ...
    def get_stock_hist_data(self, symbol, start_date=None, end_date=None, adjust='qfq'):
        ...
        try:
            ...
            logger.debug(f"[Akshare] 正在获取 {symbol} 的历史数据...")
            ...
            if df is not None and not df.empty:
                ...
                logger.debug(f"[Akshare] 成功获取 {len(df)} 条数据")
                return df
        except Exception as e:
        ...
        if self.tushare_available:
            try:
                logger.debug(f"[Tushare] 正在获取 {symbol} 的历史数据（备用数据源）...")
                ...
                if df is not None and not df.empty:
                    ...
                    logger.debug(f"[Tushare] 成功获取 {len(df)} 条数据")
                    return df
            except Exception as e:
    ...
    def get_stock_basic_info(self, symbol):
        ...
        try:
            import akshare as ak
            logger.debug(f"[Akshare] 正在获取 {symbol} 的基本信息...")
            ...
            if stock_info is not None and not stock_info.empty:
                ...
                logger.debug(f"[Akshare] 成功获取基本信息")
                return info
        except Exception as e:
            logger.warning(f"[Akshare] 获取失败: {e}")
        ...
        if self.tushare_available:
            try:
                logger.debug(f"[Tushare] 正在获取 {symbol} 的基本信息（备用数据源）...")
                ...
                if df is not None and not df.empty:
                    ...
                    logger.debug(f"[Tushare] 成功获取基本信息")
                    return info
            except Exception as e:
                logger.warning(f"[Tushare] 获取失败: {e}")
    ...
    def get_realtime_quotes(self, symbol):
        ...
        try:
            import akshare as ak
            logger.debug(f"[Akshare] 正在获取 {symbol} 的实时行情...")
            ...
            if not stock_df.empty:
                ...
                logger.debug(f"[Akshare] 成功获取实时行情")
                return quotes
        except Exception as e:
            logger.warning(f"[Akshare] 获取失败: {e}")
        ...
        if self.tushare_available:
            try:
                logger.debug(f"[Tushare] 正在获取 {symbol} 的实时行情（备用数据源）...")
                ...
                if df is not None and not df.empty:
                    ...
                    logger.debug(f"[Tushare] 成功获取实时行情")
                    return quotes
            except Exception as e:
                logger.warning(f"[Tushare] 获取失败: {e}")
    ...
    def get_financial_data(self, symbol, report_type='income'):
        ...
        try:
            import akshare as ak
            logger.debug(f"[Akshare] 正在获取 {symbol} 的财务数据...")
            ...
            if df is not None and not df.empty:
                logger.debug(f"[Akshare] 成功获取财务数据")
                return df
        except Exception as e:
            logger.warning(f"[Akshare] 获取失败: {e}")
        ...
        if self.tushare_available:
            try:
                logger.debug(f"[Tushare] 正在获取 {symbol} 的财务数据（备用数据源）...")
                ...
                if df is not None and not df.empty:
                    logger.debug(f"[Tushare] 成功获取财务数据")
                    return df
            except Exception as e:
                logger.warning(f"[Tushare] 获取失败: {e}")
