...
# 导入TDX数据源（如果可用）
try:
    from src.modules.smart_monitor.smart_monitor_tdx_data import SmartMonitorTDXDataFetcher
    TDX_AVAILABLE = True
except ImportError:
    TDX_AVAILABLE = False
...
class StockMonitorService:
        ...
        for stock in stocks:
            ...
            if last_checked:
                ...
                if current_time < next_check:
                    ...
                    logger.debug(f"股票 {stock['symbol']} 距离下次检查还有 {time_left:.1f} 分钟")
                    continue
            
            try:
                logger.info(f"正在更新股票 {stock['symbol']} 的价格...")
    ...
    def _update_stock_price(self, stock: Dict):
        ...
        try:
            ...
            if self.use_tdx and self._is_a_stock(symbol):
                logger.debug(f"使用TDX数据源获取 {symbol} 行情...")
                quote = self.tdx_fetcher.get_realtime_quote(symbol)

                if quote and quote.get('current_price'):
                    current_price = float(quote['current_price'])
                    logger.info(f"TDX获取成功: {symbol} 当前价格: ¥{current_price}")
                else:
                    # TDX失败，降级到默认数据源
                    logger.warning(f"TDX获取失败，降级到默认数据源: {symbol}")
            ...
            if current_price and current_price > 0:
                try:
                    ...
                    logger.warning(f"股票 {symbol} 价格格式错误: {current_price}")
                    # 即使失败也更新last_checked，避免持续重试
                    monitor_db.update_last_checked(stock['id'])
            else:
                logger.warning(f"无法获取股票 {symbol} 的当前价格")
    ...
    def _execute_quant_trade(self, stock: Dict, signal_type: str, current_price: float):
        ...
        try:
            ...
            if not miniqmt.is_connected():
                logger.warning(f"MiniQMT未连接，无法执行 {stock['symbol']} 的量化交易")
                return

            # 获取量化配置
            quant_config = stock.get('quant_config', {})
            if not quant_config:
                logger.warning(f"股票 {stock['symbol']} 未配置量化参数")
                return
            ...
            if success:
                logger.info(f"量化交易成功: {stock['symbol']} - {msg}")
                ...
                logger.error(f"量化交易失败: {stock['symbol']} - {msg}")

        except Exception as e:
            logger.error(f"执行量化交易异常: {stock['symbol']} - {str(e)}")
    ...
    def get_scheduler(self):
        """获取调度器实例"""
        from src.modules.monitor.monitor_scheduler import get_scheduler
