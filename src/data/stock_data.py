...
import json
import pywencai
from src.modules.monitor.data_source_manager import data_source_manager
...
class StockDataFetcher:
    ...
    def _get_chinese_stock_info(self, symbol):
        ...
        logger.info("[开始] 获取A股基本信息", symbol=symbol)
        logger.debug(f"[参数详情] symbol={symbol}")
        ...
        try:
            ...
            if basic_info:
                ...
                logger.debug(f"[数据源管理器] 获取基本信息成功", 
                            symbol=symbol,
                            name=basic_info.get('name', 'N/A'),
                            price=basic_info.get('current_price', 'N/A'))
            
            # 方法1: 尝试获取个股详细信息（akshare）
            logger.debug(f"[步骤1/4] 尝试akshare详细接口", symbol=symbol)
            try:
                stock_info = ak.stock_individual_info_em(symbol=symbol)
                if stock_info is not None and not stock_info.empty:
                    logger.debug(f"[步骤1/4] akshare详细接口成功", 
                                symbol=symbol,
                                fields_count=len(stock_info))
                ...
                logger.warning(f"[步骤1/4] akshare详细接口失败", 
                              symbol=symbol,
                              error_type=type(e).__name__,
                              error_msg=str(e)[:100])
                if self.data_source_manager.tushare_available and info['name'] == '未知':
                    logger.debug(f"[步骤1/4-备用] 尝试tushare", symbol=symbol)
                    try:
                        ...
                        if df is not None and not df.empty:
                            ...
                            logger.info(f"[步骤1/4-备用] tushare获取部分信息成功", 
                                       symbol=symbol,
                                       pe=info['pe_ratio'],
                                       pb=info['pb_ratio'])
                    except Exception as te:
                        logger.warning(f"[步骤1/4-备用] tushare获取失败",
                                      symbol=symbol,
                                      error_type=type(te).__name__,
                                      error_msg=str(te)[:100])
            ...
            # except Exception as e:
            #     logger.debug(f"[Akshare] 获取实时数据失败: {e}")
            #     # 如果实时数据获取失败，尝试使用数据源管理器获取历史数据（支持tushare备用）
            try:
                logger.debug("尝试获取最近交易数据...")
                ...
                if hist_data is not None and not hist_data.empty:
                    ...
                    if 'close' in hist_data.columns:
                        ...
                        logger.debug(f"[步骤2/4] 获取价格数据成功",
                                    symbol=symbol,
                                    price=info['current_price'],
                                    change_pct=info['change_percent'],
                                    data_points=len(hist_data))
            except Exception as e2:
                logger.warning(f"[步骤2/4] 获取历史数据失败",
                              symbol=symbol,
            ...
            if info['pb_ratio'] == 'N/A':
                try:
                    ...
                    logger.debug(f"获取市净率失败: {e}")
            
            logger.info(f"[完成] 获取A股基本信息",
                        symbol=symbol,
                        name=info.get('name'),
            ...
            logger.error(f"[失败] 获取中国股票信息完全失败",
                        symbol=symbol,
                        error_type=type(e).__name__,
                        error_msg=str(e)[:200])
            logger.debug("[失败] 详细堆栈:", exc_info=True)
            # 返回基本信息，避免完全失败
            return {
                "symbol": symbol,
                "name": f"股票{symbol}",
    ...
    def _get_chinese_stock_data(self, symbol, period="1y"):
        ...
        logger.info("[开始] 获取A股历史数据", symbol=symbol, period=period)
        logger.debug(f"[参数详情] symbol={symbol}, period={period}")
        try:
            # 计算日期范围
            end_date = datetime.now().strftime('%Y%m%d')
            if period == "1y":
            ...
            if df is not None and not df.empty:
                ...
                logger.info(f"[完成] 获取A股历史数据",
                           symbol=symbol,
                           period=period,
                           rows=len(df),
                           date_range=f"{df.index[0]} ~ {df.index[-1]}" if len(df) > 0 else "N/A")
                return df
            else:
                logger.warning(f"[失败] 所有数据源均无法获取历史数据", symbol=symbol)
                return {"error": "所有数据源均无法获取历史数据"}
                
        except Exception as e:
            logger.error(f"[失败] 获取中国股票历史数据异常",
                        symbol=symbol,
                        period=period,
                        error_type=type(e).__name__,
                        error_msg=str(e)[:200])
    ...
    def calculate_technical_indicators(self, df):
        ...
        try:
            ...
            logger.debug("[开始] 计算技术指标", rows=rows, indicators=['MA5', 'MA10', 'MA20', 'MA60', 'RSI', 'MACD', 'KDJ', 'BOLL', 'Volume_MA5'])
            ...
            logger.debug("[完成] 计算技术指标", 
                       indicators_added=len(['MA5', 'MA10', 'MA20', 'MA60', 'RSI', 'MACD', 'MACD_signal', 'MACD_histogram', 'BB_upper', 'BB_middle', 'BB_lower', 'K', 'D', 'Volume_MA5', 'Volume_ratio']))
            return df
            
        except Exception as e:
            logger.error("[失败] 计算技术指标失败",
                        error_type=type(e).__name__,
                        error_msg=str(e)[:200])
            logger.debug("[失败] 详细堆栈:", exc_info=True)
    ...
    def _get_chinese_financial_data(self, symbol):
        ...
        try:
            # 1. 获取资产负债表
            try:
                balance_sheet = ak.stock_financial_abstract_new_ths(symbol=symbol, indicator="资产负债表")
                if balance_sheet is not None and not balance_sheet.empty:
                    financial_data["balance_sheet"] = balance_sheet.head(8).to_dict('records')
            except Exception as e:
                logger.error(f"获取资产负债表失败: {e}", exc_info=True)
            
            # 2. 获取利润表
            try:
                income_statement = ak.stock_financial_abstract_new_ths(symbol=symbol, indicator="利润表")
            ...
            # 3. 获取现金流量表
            try:
                cash_flow = ak.stock_financial_abstract_new_ths(symbol=symbol, indicator="现金流量表")
            ...
            # 4. 获取主要财务指标
            try:
                financial_abstract = ak.stock_financial_abstract_new_ths(symbol=symbol)
        ...
        try:
            ...
            try:
                ...
                if financial_indicator is not None and not financial_indicator.empty:
                    ...
                    logger.debug(f"ROE: {financial_data['financial_ratios']['ROE股东权益回报率']}")
                    logger.debug(f"市盈率: {financial_data['financial_ratios']['市盈率']}")
                    logger.debug(f"市净率: {financial_data['financial_ratios']['市净率']}")
                else:
                    logger.warning(f"未获取到港股 {hk_code} 的财务指标数据")
                    financial_data["note"] = "未获取到财务数据"
                    
            except Exception as e:
                logger.warning(f"获取港股财务指标失败: {e}")
