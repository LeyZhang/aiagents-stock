...
logger = get_logger("fund_flow_data")
...
class FundFlowAkshareDataFetcher:
    ...
    def get_fund_flow_data(self, symbol):
        ...
        try:
            logger.info(f"正在获取 {symbol} 的资金流向数据...")
            ...
            if fund_flow_data:
                ...
                logger.info(f"成功获取 {len(fund_flow_data.get('data', []))} 个交易日的资金流向数据")
                data["data_success"] = True
                logger.info("资金流向数据获取完成")
            else:
                logger.warning("未能获取到资金流向数据")
    ...
    def _get_individual_fund_flow(self, symbol, market):
        ...
        try:
            ...
            logger.debug(f"正在获取资金流向 (市场: {market})...")
            
            df = ak.stock_individual_fund_flow(stock=symbol, market=market)
            
            if df is None or df.empty:
                logger.debug("未找到资金流向数据，尝试备用数据源...")
                
                # akshare失败，尝试tushare
                if data_source_manager.tushare_available:
                    try:
                        logger.debug("正在获取资金流向数据（备用数据源）...")
                        ...
                        if df is not None and not df.empty:
                            ...
                            logger.warning("未找到资金流向数据")
                            return None
                    except Exception as te:
                        logger.error(f"获取失败: {te}")
                        return None
                else:
                    return None
...
if __name__ == "__main__":
    ...
    for symbol, name in test_symbols:
        ...
        if data.get("data_success"):
            ...
            logger.error(f"获取失败: {data.get('error', '未知错误')}")
