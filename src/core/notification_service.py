...
class NotificationService:
    ...
    def __init__(self):
        logger.debug("初始化 NotificationService")
        # 强制重新加载环境变量
        from dotenv import load_dotenv
        load_dotenv()
        self.config = self._load_config()
        logger.debug(f"通知配置: email_enabled={self.config['email_enabled']}, webhook_enabled={self.config['webhook_enabled']}")
    ...
    def send_notifications(self):
        ...
        for notification in notifications:
            try:
                logger.debug(f"处理通知: {notification['symbol']} - {notification['type']}")
    ...
    def send_notification(self, notification: Dict) -> bool:
        ...
        if not success:
            try:
                ...
                logger.debug("界面通知不可用（非Streamlit环境）")
    ...
    def _send_email_notification(self, notification: Dict) -> bool:
        ...
        try:
            ...
            if not all([self.config['smtp_server'], self.config['email_from'],
                ...
                logger.warning("邮件配置不完整，使用界面通知")
                logger.debug(f"SMTP服务器: {self.config['smtp_server'] or '未配置'}")
                logger.debug(f"发件人: {self.config['email_from'] or '未配置'}")
                logger.debug(f"收件人: {self.config['email_to'] or '未配置'}")
                logger.debug(f"密码: {'已配置' if self.config['email_password'] else '未配置'}")
                self._show_streamlit_notification(notification)
                return True
            ...
            logger.debug(f"正在发送邮件...")
            logger.debug(f"收件人: {self.config['email_to']}")
            logger.debug(f"主题: 股票监测提醒 - {notification['symbol']}")

            # 根据端口选择连接方式
            if self.config['smtp_port'] == 465:
                logger.debug(f"使用 SMTP_SSL 连接 {self.config['smtp_server']}:{self.config['smtp_port']}")
                server = smtplib.SMTP_SSL(self.config['smtp_server'], self.config['smtp_port'], timeout=15)
            else:
                logger.debug(f"使用 SMTP+TLS 连接 {self.config['smtp_server']}:{self.config['smtp_port']}")
    ...
    def _show_streamlit_notification(self, notification: Dict):
        ...
        try:
            ...
            # 不在Streamlit环境中，静默跳过
            logger.debug("跳过Streamlit通知（非Streamlit环境）")
        except Exception as e:
            logger.warning(f"Streamlit通知显示失败: {e}")
...
_此消息由AI股票分析系统自动发送_"""
            ...
            logger.debug(f"[钉钉] 正在发送Webhook...")
            logger.debug(f"URL: {self.config['webhook_url'][:50]}...")
            ...
            if response.status_code == 200:
                ...
                if result.get('errcode') == 0:
                    logger.info("[钉钉] Webhook发送成功")
                    return True
                else:
                    logger.warning(f"[钉钉] Webhook返回错误: {result.get('errmsg')}")
                    return False
            else:
                logger.warning(f"[钉钉] Webhook请求失败: HTTP {response.status_code}")
                return False

        except Exception as e:
            logger.error(f"[钉钉] Webhook发送异常: {e}")
            return False
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if result.get('code') == 0:
                    logger.info("[飞书] Webhook发送成功")
                    return True
                else:
                    logger.warning(f"[飞书] Webhook返回错误: {result.get('msg')}")
                    return False
            else:
                logger.warning(f"[飞书] Webhook请求失败: HTTP {response.status_code}")
                return False

        except Exception as e:
            logger.error(f"[飞书] Webhook发送异常: {e}")
            return False
...
"""
            ...
            if self.config['webhook_enabled']:
                ...
                if webhook_success:
                    success = True
                    logger.info("[Webhook] Webhook通知发送成功")

            return success

        except Exception as e:
            logger.error(f"[ERROR] 发送持仓分析通知失败: {str(e)}", exc_info=True)
            return False
    ...
    def _send_custom_email(self, subject: str, html_body: str, text_body: str) -> bool:
        ...
        try:
            ...
            logger.warning(f"[邮件] 邮件发送失败: {str(e)}")
            return False
    ...
    def _send_portfolio_webhook(self, analysis_results: dict, sync_result: dict = None) -> bool:
        ...
        try:
            ...
            logger.error(f"[ERROR] Webhook发送失败: {str(e)}")
            return False
